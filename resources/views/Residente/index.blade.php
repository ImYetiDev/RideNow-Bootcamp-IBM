        ...
        <div class="container-fluid pt-4 px-4">
            <div class="bg-secondary text-center rounded p-4">
                <div class="d-flex align-items-center justify-content-between mb-4">
                    <h6 class="mb-0">TABLA RESIDENTES</h6>
                    <a href="{{ route('Residente.create') }}">
                    ...
                    <table class="table text-start align-middle table-bordered table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $dato->movil_propietario}}</td>
                                <td>{{ $dato->email_propietario}}</td>
                                <td>{{ $dato->estado}}</td>
                                <td>{{ $dato->created_at }}</td>
                                <td>{{ $dato->updated_at }}</td>
                                <td>
                                    <a href="{{ route('Residente.edit', $dato->id) }}" class="btn btn-warning" style="display: inline-block">
                                        Editar</a>
                                </td>
                                <td>
                                    <form action="{{ route('Residente.destroy', $dato->id) }}" method="POST" style="display: inline-block">
                                        @csrf
                                        @method('DELETE')
