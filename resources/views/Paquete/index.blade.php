            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">TABLA PAQUETES</h6>
                    ...
                    <table class="table text-start align-middle table-bordered table-hover mb-0">
                        ...
                        <tbody>
                        @foreach($Paquete as $dato)
                            <tr>
                                <td>{{ $dato->id }}</td>
                                    <td>{{ $dato->destinatario }}</td>
                                    <td>{{ $dato->vivienda_id }}</td>
                                    <td>{{ $dato->recibido_por }}</td>
                                    <td>{{ $dato->entregado_a }}</td>
                                    <td>{{ $dato->estado }}</td>
                                    <td>{{ $dato->created_at }}</td>
                                    <td>{{ $dato->updated_at }}</td>
                                    <td>
                                    <a href="{{ route('Paquete.edit', $dato->id) }}" class="btn btn-warning" style="display: inline-block">
                                        Editar</a>
                                </td>
                                <td>
                                    <form action="{{ route('Paquete.destroy', $dato->id) }}" method="POST" style="display: inline-block">
                                        @csrf
                                        @method('DELETE')
