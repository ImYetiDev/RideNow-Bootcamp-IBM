            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">TABLA RESERVA</h6>
                    ...
                    <table class="table text-start align-middle table-bordered table-hover mb-0">
                        <thead>
                            <tr>
                                <th scope="col">ID</th>
                                <th scope="col">Zona Comun Id</th>
                                <th scope="col">Fecha Reserva</th>
                                <th scope="col">Hora Reserva</th>
                                <th scope="col">Residente Id</th>
                        ...
                        @foreach($Reserva as $dato)
                            <tr>
                                <td>{{ $dato->id }}</td>
                                    <td>{{ $dato->zona_comun_id }}</td>
                                    <td>{{ $dato->fecha_reserva }}</td>
                                    <td>{{ $dato->hora_reserva }}</td>
                                    <td>{{ $dato->residente_id }}</td>
                                    <td>{{ $dato->estado }}</td>
                                    <td>{{ $dato->created_at }}</td>
                                    <td>{{ $dato->updated_at }}</td>
                                    <td>
                                    <a href="{{ route('Reserva.edit', $dato->id) }}" class="btn btn-warning" style="display: inline-block">
                                        Editar</a>
                                </td>
                                <td>
                                    <form action="{{ route('Reserva.destroy', $dato->id) }}" method="POST" style="display: inline-block">
                                        @csrf
                                        @method('DELETE')
